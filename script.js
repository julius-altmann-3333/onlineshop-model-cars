...
function renderBrands() {
    ...
    container.innerHTML = brands.map(brand => `
        <button onclick="filterByBrand('${brand.id}')" class="btn btn-outline-secondary brand-btn" id="btn-${brand.id}">
            <img src="${brand.logo}" width="24" class="me-2"> ${brand.name}
        </button>
    `).join('');
}
...
function renderProducts(filteredProducts = products) {
    ...
    container.innerHTML = filteredProducts.map(p => `
        <div class="col-md-6 col-lg-4 mb-4" data-brand="${p.brand}">
            <div class="card h-100 shadow-sm border-0" style="cursor:pointer;" onclick="openModal(${p.id})">
                <img src="${p.image}" class="card-img-top" alt="${p.name}">
                <div class="card-body">
                    <h6 class="card-subtitle mb-2 text-warning">${p.brand.toUpperCase()}</h6>
...
function openModal(id) {
    ...
    document.getElementById('modalBrand').textContent = currentProduct.brand.toUpperCase();
    document.getElementById('modalDesc').textContent = currentProduct.desc || "Premium 1:64 diecast model car.";
    document.getElementById('modalPrice').textContent = `$${currentProduct.price.toFixed(2)}`;
    document.getElementById('modalRating').textContent = currentProduct.rating;
...
document.addEventListener('keypress', e => {
    if (e.key >= '1' && e.key <= '9') {
        ...
        if (brands[idx]) filterByBrand(brands[idx].id);
    }
});
